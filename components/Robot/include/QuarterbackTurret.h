...
// const float flywheelSpeeds[QB_TURRET_NUM_SPEEDS] = {-0.1, 0, 0.1, 0.225,
// 0.35, 0.45, 1.0}; // with top prongs

// Preset flywheel speed values.
const float flywheelSpeeds[QB_TURRET_NUM_SPEEDS] = {
    -0.1, 0, 0.1, 0.215, 0.31, 0.3875, 1.0};  // without top prongs
const float combineSpeeds[QB_TURRET_NUM_COMBINE_SPEEDS] = {
    -0.1, 0, 0.215, 0.35, 0.45};  // combine speed values
...
// QB_COUNTS_PER_TURRET_REV       Encoder Gear: 18t; Turret Track: 152t. Encoder
// spins 8.4 times for every turret revolution = 8500 ticks/rev
// QB_COUNTS_PER_TURRET_DEGREE    (8500/360) = 23.61... ticks per degree
// QB_TURRET_SLOP_COUNTS          Backlash between input and output on the
// turret is high -> leads to problems when switching directions
//                                    540 ticks on encoder before turret
//                                    actually starts to move (Empirically
//                                    measured)
#define QB_COUNTS_PER_ENCODER_REV 1250
#define QB_COUNTS_PER_TURRET_REV \
  10556  // actually 10555.55555...6, but rounded up
...
// #define QB_ENCODER_TO_TURRET_RATIO  /* = */ QB_FALCON_TO_TURRET_RATIO /
// QB_ENCODER_TO_FALCON_RATIO // (27 / 5) = 5.4 #define QB_COUNTS_PER_TURRET_REV
// /* = */ QB_ENCODER_TO_TURRET_RATIO * QB_COUNTS_PER_ENCODER_REV // 5400
...
// #define QB_TURRET_SLOP_PCT          /* = */
// QB_DIRECTION_CHANGE_SLOP_GEAR_TEETH / 100 #define QB_TURRET_SLOP_COUNTS /* =
...
// QB_TURRET_STOP_THRESHOLD_MS          must be a multiple of
// QB_TURRET_STOP_LOOP_DELAY_MS QB_TURRET_HOME_STOP_FACTOR           correction
// constant for homing, multiplied into stop counts in final homing
// QB_TURRET_MANUAL_CONTROL_FACTOR      higher values = less sensitive during
// manual control (Basically divides the clock of the loop)
...
// sets reciever 0's ID
const unsigned long long RECEIVER_0_ID = 0xDECA4B5BCBB00FA3;

//=====================================//
//   Turret PID Controller Constants   //
//=====================================//
// QB_TURRET_PID_THRESHOLD      Acceptable error in position control (degrees)
// that will zero the error constants / build up QB_TURRET_PID_MIN_DELTA_T    If
// PID error values are not updated fast enough, for example if the controller
// hangs up for half a second,
//                                  then the PWM values will be massive compared
//                                  to what they should be so just kind of reset
//                                  everything
...
// QB_NORTH_OFFSET              Added to deal with the problems of zeroing the
// turret but then holding a set angle afterwards
...
#define NUM_RECEIVERS 4
  ...
  //==============================//
  // enabled        default false, set true when homing / reset complete
  // (toggleable via function) initialized    default false, set true when
  // homing / reset complete (stays true) ->
  //                    decides whether or not to just run zero turret or entire
  //                    setup routine
  ...
  // options: [straight, angled]. targetAssemblyAngle    defaults to [straight].
  // options: [straight, angled]. assemblyMoving         defaults to false
  ...
  // options: [forward, back]. targetCradleState    defaults to [back]. options:
  // [forward, back]. cradleMoving         defaults to [false] cradleStartTime
  ...
  // flywheelManualOverride     defaults to false, true when stick is
  // controlling flywheel
  ...
  int8_t utmsCtr = 0;  // temp ctr for debugging so updateTurretMotionStatus
                       // doesn't spam as much
#define UTMS_CTR_MAX 15
  ...
  // targetTurretEncoderCount       default = 0. complementary to
  // [currentTurretEncoderCount] errorEncoderCount              the error
  // [currentTurretEncoderCount - targetTurretEncoderCount] slopError any error
...
#pragma region Public
 ...
 public:
  ...
  // reset                    zero turret, aim down (straight), and set
  // flywheels to slow intake
  void zeroTurret();
  void reset();

  // [Safety]
  // setEnabled               toggles turret and flywheel movement
  // emergencyStop            If the E-Stop button on controller is pressed this
  // should override every other motor function and force them to stop
  // testForDisableOrStop     Used for safety, returns false if the robot is
  // disabled or should be stopped
  void setEnabled(bool enabled);
  void emergencyStop();
  bool testForDisableOrStop();

  // [Debug]
  // printDebug               Consolidating printouts for the different
  // functions into one locations for debugging purposes testRoutine Used for
  // debugging
  void printDebug();
  void testRoutine();
  ...
  //====================================//
  // setTurretSpeed                 Moves the turret at a specified speed (open
  // loop) moveTurret                     Turn the turret to a specific heading
  // (currently relative to robot) -> asynchronous moveTurretAndWait Moves the
  // turret and waits until the heading is reached -> synchronous aimAssembly
  // Moves the falcon pylon assembly up and down between two states moveCradle
  // Moves the cradle between two states throw and hold setFlywheelSpeed Sets
  // the speed of the flywheels using stick inputs setFlywheelSpeedStage Set the
  ...
  //===================================//
  // switchMode                 Used to switch between manual and automatic mode
  // (overloaded function) switchTarget               Switch between receiver1
  // or receiver2
  ...
  //==================================//
  // loadFromCenter             Prepares the QB to accept the ball from the
  ...
  // * any variables used in the turretEncoderISR() (interrupt service routine)
  // must be public static
  static int32_t
      currentTurretEncoderCount;  // updated by encoder ISR (interrupt service
                                  // routine). complementary to
                                  // [targetTurretEncoderCount].
  static uint8_t
      turretEncoderStateB;  // A channel will always be 1 when interrupt
                            // triggers, so only care about channel B

  //===============================================//
  //    Encoder Interrupt Service Routine (ISR)    //
  //===============================================//
  // must be static. simple function called when encoder interrupts are
  // triggered. updates [currentTurretEncoderCount].
  static void turretEncoderISR();

  // If the direction changes then some things need to happen differently
  // because of the incredible amount of backlash
